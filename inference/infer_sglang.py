...
def stage2_generate(engine, prompt, codectool, codectool_stage2, mmtokenizer, stage2_logit_processor, batch_size=16):
...
def stage2_inference(engine, stage1_output_set, stage2_output_dir, codectool, codectool_stage2, mmtokenizer, stage2_logit_processor, batch_size=4):
    ...
    for i in tqdm(range(len(stage1_output_set)), desc="Stage 2 inference..."):
        ...
        if num_batch <= batch_size:
            output = stage2_generate(engine, prompt[:, :output_duration*50], codectool, codectool_stage2, mmtokenizer, stage2_logit_processor, batch_size=num_batch)
        else:
            segments = []
            ...
            for seg in range(num_segments):
                ...
                segment = stage2_generate(engine, prompt[:, start_idx:end_idx], codectool, codectool_stage2, mmtokenizer, stage2_logit_processor, batch_size=current_batch_size)
        ...
        if output_duration * 50 != prompt.shape[-1]:
            ending = stage2_generate(engine, prompt[:, output_duration*50:], codectool, codectool_stage2, mmtokenizer, stage2_logit_processor, batch_size=1)
...
def main():
    """Main function - must be called from if __name__ == '__main__' for multiprocessing"""
    ...
    stage2_result = stage2_inference(
        stage2_engine, stage1_output_set, stage2_output_dir, 
        codectool, codectool_stage2, mmtokenizer, stage2_logit_processor,
